...
def load_activities():
    dicc_core = {
        ...
        "Daily productivity": [
            ...
            {
                "core_activity": "Writing",
                "color": "#CC99FF",
                "activities": [
                    "Writing reports",
                    "Editing documents",
                    "Taking notes",
                    "Writing in Word or Docs"
                ]
            },
            {
                "core_activity": "Reading",
                "color": "#99CC99",
                "activities": [
                    "Reading articles",
                    "Reading PDFs",
...
# Nueva función para generar reglas heurísticas (optimizada)
def generate_heuristic_rules():
    dicc_core, _, _, _ = load_activities()
    rules = []

    stopwords = {"the", "for", "with", "and", "of", "in", "to", "a", "on", "by", "from", "at", "an", "as", "is", "that", "are"}

    for category in dicc_core.values():
        for activity in category:
            core = activity["core_activity"]
            for sub in activity["activities"]:
                # Normalizamos y dividimos en palabras clave limpias
                terms = [
                    word.lower()
                    for word in re.findall(r'\b\w+\b', sub)
...
# 👇 Esto va fuera de cualquier función
if __name__ == "__main__":
    ...
    print("\n===== EJEMPLOS DE ACTIVIDADES (Education) =====")
    for activity in dicc_core.get("Education", []):
        print(f"\nActividad principal: {activity['core_activity']}")
        print("Subactividades:")
        for sub in activity['activities']:
            print(f"  - {sub}")

    print("\n===== MAPA DE SUBACTIVIDADES A ACTIVIDAD PRINCIPAL =====")
    count = 0
    for subact, core in dicc_map_subact.items():
        print(f"{subact} --> {core}")
        count += 1
        if count >= 10:
            break

    print("\n===== COLORES ASOCIADOS A ACTIVIDADES =====")
    for core_act, color in dicc_core_color.items():
        print(f"{core_act}: {color}")
